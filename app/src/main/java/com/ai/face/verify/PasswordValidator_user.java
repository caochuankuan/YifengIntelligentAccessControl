...
public class PasswordValidator_user extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_password_validator_user);
        ...
        correctPassword = sp.getString("mima", default_value);
        SharedPreferences.Editor editor = sp.edit();

        if (correctPassword.isEmpty()) {
            // 如果密码不存在，写入默认密码
            editor.putString("mima", "123"); // 这里是默认密码
            editor.apply();
            correctPassword = "123"; // 将 correctPassword 设置为默认密码
        }
        ...
        forget.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                new AlertDialog.Builder(PasswordValidator_user.this).setTitle("温馨提示").setMessage("默认密码如已修改，请联系小区管理人员。\nQQ:2835082172@qq.com").show();
            }
        });
    ...
    private void onButtonClick(Button button) {
        ...
        if (enteredPassword.length() == correctPassword.length()) {
            // 验证密码
            if (enteredPassword.toString().equals(sp.getString("mima", correctPassword))) {
                tvEnteredDigits.setText("");
                // 密码正确
                Toast.makeText(this, "密码正确", Toast.LENGTH_SHORT).show();
                startPlay();
                startActivity(new Intent(PasswordValidator_user.this,com.ai.face.search.FaceSearch1NActivity.class));
                finish();
            } else {
                // 密码错误
