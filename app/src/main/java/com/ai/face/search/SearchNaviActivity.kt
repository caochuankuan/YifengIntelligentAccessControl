...
class SearchNaviActivity : AppCompatActivity(), PermissionCallbacks {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.zhuti.setOnClickListener {
            startActivity(
                Intent(this@SearchNaviActivity, SearchNaviActivity2::class.java)
            )
            finish()
        }
        ...
        binding.aboutus.setOnClickListener {
            startActivity(
                Intent(this@SearchNaviActivity, com.ai.face.ui.about::class.java)
            )
        }
        ...
        binding.showImg.setOnClickListener {
            startActivity(
                Intent(this@SearchNaviActivity, com.ai.face.ui.DisplayFacesActivity::class.java)
            )
        }
...
//        binding.deviceInfo.text="设备指纹:"+ DeviceFingerprint.getDeviceFingerprint()
    ...
    companion object {
        ...
        //ftp服务器的照片写入数据库
        suspend fun copyManyTestFaceImages1(context: Application) = withContext(Dispatchers.IO) {
            val cacheDir = File(FaceApplication.CACHE_SEARCH_FACE_DIR)
            ...
            if (cacheFiles != null) {
                for (file in cacheFiles) {
                    // 插入照片
                    FaceSearchImagesManger.c.getInstance(context)?.insertOrUpdateFaceImage(
                        getBitmapFromFile(file.absolutePath),
                        file.absolutePath
                    )
                }
            }
